 ...
 <template>
     ...
     <h3 class="text-2xl mb-4">Create new order</h3>
                ...
                <ul ref="list" class="list" v-sortable="{animation: 250, onUpdate: work}">
                    <li v-for="(item, index) in orderedItems" :_id="item.id" :order="item.order" :key="item.id" >
                        <div class="grid grid-cols-3 gap-3">
                            <div ><input placeholder="Enter quantity" type="number" @change="setQ($event, item.id)"/></div>
                            <div >
                            <select
 ...
 export default {
    ...
    methods: {
        ...
        setQ($event, item_id) {
            ...
            this.form.items = this.orderedItems
        },
        setP($event, item_id) {
